"""Qwen3-VL with a causal scalar head and sparse, score-conditioned rationale SFT."""
...
class JointQwen3VLForConditionalGeneration(Qwen3VLForConditionalGeneration):
    ...
    Scores use 0..9 at the interface and 0..1 for Huber loss. Rationale CE projects
    only supervised token states to vocabulary logits, in checkpointed chunks.
    """

    def __init__(self, config):
        super().__init__(config)
        defaults = {"rationale_weight": 0.1, "huber_delta": 0.1,
        ...
        if defaults["rationale_weight"] < 0 or defaults["huber_delta"] <= 0:
            raise ValueError("Invalid joint loss weights")
        ...
        self.score_head = nn.Linear(config.text_config.hidden_size, 1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        normalized = torch.sigmoid(self.score_head(pooled).float()).squeeze(-1)
        ...
        if scores is not None or labels is not None:
            loss = score_loss + self.config.judge_config["rationale_weight"] * rationale_loss
        return JointJudgeOutput(
            loss=loss, logits=predicted[:, None], score_loss=score_loss,
